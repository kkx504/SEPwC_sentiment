...
options(lintr.linters = lintr::linters_with_defaults(
  line_length_linter = NULL #because i have lines longer than 88 due to comments
))

load_data <- function(filename,
                      string_as_factors = FALSE,
                      verbose = FALSE) { #we need this to do string manipulation
  if (verbose) message("Reading data...")
  data <- read.csv(filename, colClasses = c("id" = "character")) #id read as a character
...
word_analysis <- function(toot_data, emotion, verbose = FALSE) {
  ...
  word_data <- tail(words_with_sentiment %>% arrange(desc(id)), 9) %>% ungroup()#9 because the bottom 9 have the desired IDs
...
sentiment_analysis <- function(toot_data, verbose = FALSE) {
  ...
  nrc_sentiment <- (get_sentiments("nrc")) %>%
    ...
    summarise(sentiment = paste(unique(sentiment), collapse = ",", .groups = "drop")) %>% #sentiments can be an emotion as well as positive/negative, so if both can seperate this with a ,
    ungroup() %>%
    mutate(method = "nrc") %>%
  ...
  bing_sentiment <- (get_sentiments("bing")) %>%
    inner_join(sentiment_data, by = "word", relationship = "many-to-many") %>%
    group_by(id, created_at) %>% #nolint: object_usage_linter
    summarise(sentiment = first(sentiment), .groups = "drop") %>% #if there is more than one, choose the first one
    ungroup() %>%
    mutate(method = "bing") %>%
    select(id, created_at, method, sentiment) #nolint: object_usage_linter
...
main <- function(args) {
  ...
  word_result <- word_analysis(toot_data, args$emotion) #nolint: object_usage_linter
